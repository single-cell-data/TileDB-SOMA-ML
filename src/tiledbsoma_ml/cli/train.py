...
@option('-S', '--seed', type=int, help='PRNG seed')
@option('-t', '--tissue', help='"tissue_general" obs filter')
@option('-v', '--census-version', default=DEFAULT_CENSUS_VERSION, help=f'Census version to train on (default: {DEFAULT_CENSUS_VERSION})')
@option('-w', '--num-workers', type=int, help="Number of DataLoader workers")
@option('--local-rank', type=int, help='Used for distributed training (automatically set by torch.distributed.launch)')
def train(
    batch_size: int,
    io_batch_size: int,
